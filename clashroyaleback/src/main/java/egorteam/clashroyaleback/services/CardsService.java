...
import egorteam.clashroyaleback.repositories.CardsRepository;
...
public class CardsService {
    ...
    @Transactional
    public CardsPageResponse list(String rarity, Integer elixirMin, Integer elixirMax, Integer arena,
                                       String search, int page, int size) {
        seedFallbackCardsIfEmpty();
        int safePage = Math.max(page, 0);
        int safeSize = Math.max(1, Math.min(size, 100));
        List<Card> filtered = cards.findAll().stream()
                .map(this::toDto)
        ...
        return new CardsPageResponse(safePage, safeSize, filtered.size(), totalPages, filtered.subList(from, to));
    }
    ...
    public CardShort shortCard(Integer cardId) {
        Card card = get(cardId);
        return new CardShort(card.id(), card.apiCardId(), card.name(), card.iconUrl());
    }

    private Card toCard(Map<String, Object> source) {
        ...
        return new Card(id, apiId, name, elixir, string(source.get("rarity")), null, iconUrl, null, source, Instant.now());
    }

    private Card toDto(CardEntity entity) {
        return new Card(entity.id, entity.apiCardId, entity.name, entity.elixir, entity.rarity, entity.arena,
    ...
    private void addSeed(int id, String name, int elixir, String rarity) {
        ...
        cards.save(toEntity(new Card(id, id, name, elixir, rarity, null, null, null, Map.of(), Instant.now())));
    }
}
