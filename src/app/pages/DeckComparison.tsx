...
function DeckPanel({ title, subtitle, cards, avgElixir, winRate, synergy, color }: any) {
  return (
    <div className="bg-card border border-border rounded-xl p-6 shadow-xl">
      <div className="mb-4">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        {cards.map((card: any, index: number) => (
          ...
          >
            <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${color} mb-2 flex items-center justify-center shadow-lg text-sm`}>
              ⚔️
            </div>
...
function ComparisonRow({ label, valueA, valueB, favorA }: any) {
  return (
    <div className="flex items-center gap-4 p-3 rounded-lg bg-background/30">
      <div className={`flex-1 text-right ${favorA ? "text-primary" : "text-muted-foreground"}`}>
        {valueA}
      </div>
